...
export const data = new SlashCommandBuilder()
  .setName("rank")
  .setDescription("Check a Roblox user's current group rank")
  .addStringOption((opt) =>
    opt
      .setName("username")
      .setDescription("Roblox username to check (leave empty to check yourself)")
      .setRequired(false)
  );
      ...
      await interaction.editReply({
        embeds: [
          new EmbedBuilder()
            ...
            .setTitle("Not Linked")
            .setDescription(
              "You haven't linked your Roblox account yet. Use `/link` or provide a username."
            ),
        ],
      });
      return;
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `**${robloxUsername}** is not a member of the group, or the rank could not be retrieved.`
          ),
      ],
    });
    return;
